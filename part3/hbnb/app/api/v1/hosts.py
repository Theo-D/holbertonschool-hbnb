...
class HostList(Resource):
    @jwt_required()
    @ns.doc("list_hosts", description="List all host accounts (Admin only)", security='BearerAuth')
    ...
    @ns.doc("create_host", description="Create a new host (if first user created is admin, then admin-only)")
...
class HostDetail(Resource):
    @jwt_required()
    @ns.doc("get_host", description="Retrieve host details (Self or Admin)", security='BearerAuth')
    ...
    @jwt_required()
    @ns.doc("update_host", description="Update host profile (Self or Admin)", security='BearerAuth')
    ...
    @jwt_required()
    @ns.doc("delete_host", description="Delete a host (Self or Admin)", security='BearerAuth')
...
class HostOwnedPlaces(Resource):
    @ns.doc("get_host_owned_places", description="List places owned by host (Public)", security=[])
